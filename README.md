...
Muninn is a standalone forensic tool for log analysis, SIGMA-based threat detection, and incident response. One binary, 15+ log formats, 3100+ SIGMA rules, MITRE ATT&CK mapping (370+ techniques), anomaly detection, login analysis, executive summary with automated verdict, attack correlation, IOC extraction & enrichment, evidence integrity (SHA-256), and real-time monitoring — zero external dependencies. No SIEM required.
...
```
  ...
  ██      ██   ██████   ██    ███  ██  ██    ███  ██    ███
              Memory of Corax
  -= SIGMA Detection Engine for EVTX/JSON/Syslog/CEF/Zeek v0.5.0 =-
...
| **Сетевые сенсоры** | Zeek/Bro (dns, http, conn, ssl), Suricata EVE JSON, Snort (требуется один из поддерживаемых базовых форматов: JSON, CSV, Syslog, CEF, LEEF) |
| **Межсетевые экраны** | iptables, Palo Alto, Fortinet, Check Point (требуется один из поддерживаемых базовых форматов: CSV, Syslog, CEF, LEEF) |
...
| **Библиотека + CLI** | Rust-крейт или CLI-утилита |

#### v0.4.0 — Продвинутая аналитика
...
| **Извлечение IOC** | `--ioc-extract [FILE]` | IP, домены, URL, хэши, email, пути файлов, реестр, службы, задачи, пайпы |
| **Обогащение IOC** | `--vt-key` / `--abuseipdb-key` / `--opentip-key` | VirusTotal, AbuseIPDB, Kaspersky OpenTIP |
| **Анализ логонов** | `--login-analysis [FILE]` | Анализ аутентификации: success/fail, brute force, нетипичные часы, lateral movement, privilege escalation |
| **Executive summary** | `--summary [FILE]` | Автоматический вердикт (Clean/Suspicious/Compromised/Breach) с рекомендациями |
| **Целостность улик** | *(авто)* | SHA-256 хеширование исходных файлов, включается в JSON-отчёт |
| **Threat scoring** | `--threat-score [FILE]` | Оценка рисков по хостам/пользователям (абсолютная шкала 0-100) |
| **Корреляция атак** | `--correlate [FILE]` | Группировка детектов в цепочки атак с вычислением длительности |
| **Diff-режим** | `--diff /path/second/` | Сравнение двух наборов данных |
| **Трансформации** | `--transforms` | Base64-декодирование, LOLBin-детект (44 бинарника), DNS-энтропия, обфускация |
